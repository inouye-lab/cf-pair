...
class GBetaVAE(nn.Module):
    ...
    def __init__(self, hparam):
        ...
        self.inter_dim = hparam['inter_dim']
        self.device = hparam['device']
        self.hparam = hparam

        self.encoder = nn.Sequential(
            nn.Conv2d(self.hparam['image_shape'][0], 32, 4, 1, 0),          # B,  32, 25, 25
            nn.ReLU(True),
            nn.Conv2d(32, 32, 4, 1, 0),          # B,  32, 22, 22
            nn.ReLU(True),
            nn.Conv2d(32, 64, 4, 2, 0),          # B,  64,  10, 10
            ...
            nn.ReLU(True),
            View((-1, 256*1*1)),                 # B, 256
            nn.Linear(256, self.latent_dim),             # B, latent_dim
        )
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(64, 32, 4, 2, 0), # B,  32, 22, 22
            nn.ReLU(True),
            nn.ConvTranspose2d(32, 32, 4, 1, 0), # B,  32, 25, 25
            nn.ReLU(True),
            nn.ConvTranspose2d(32, self.hparam['image_shape'][0], 4, 1, 0),  # B, nc, 28, 28
            nn.Sigmoid()
        )
...
class GIndpendentBetaVAE(nn.Module):
    ...
    def __init__(self, hparam) -> None:
        ...
        self.encoder_dict = nn.ModuleDict({
            str(d_idx):  nn.Sequential(
                        ...
                        View((-1, 256*1*1)),                 # B, 256
                        nn.Linear(256, self.latent_dim),             # B, latent_dim
                    ) for d_idx in range(self.hparam['n_domains'])
        })
    ...
    def z_to_x(self, z, d):
        ...
        out = torch.zeros((batch_size, *self.hparam['image_shape']), device=self.device)
        for d_idx in range(self.hparam['n_domains']):
            out[d==d_idx] = self.decoder_dict[str(d_idx)](z[d==d_idx])
        return out


class GDeep(nn.Module):
    """A basic VAE model
    Loosely based on https://github.com/AntixK/PyTorch-VAE/blob/master/models/vanilla_vae.py"""
    def __init__(self, hparam):
        super().__init__()
        self.latent_dim = hparam['latent_dim']
        self.inter_dim = hparam['inter_dim']
        self.device = hparam['device']
        self.hparam = hparam
        self.generator = torch.Generator(device=self.device).manual_seed(hparam['seed'])
        self._build_models(hparam)

    def _build_models(self, hparam):
        """
        Builds the encoder and decoder models.
        Encoder: encoder, fc_mu, fc_var
        Decoder: fc_decoder, decoder
        """
        kernel = 3
        stride = 2
        padding = 0
        ...
        # Build the encoder
        modules = []
        cnn_layer_sizes = [self.inter_dim, self.inter_dim*3, self.inter_dim*5]
        self.cnn_layer_sizes = cnn_layer_sizes
        in_channels = hparam['image_shape'][0]
        for layer_size in cnn_layer_sizes:
            modules.append(
                nn.Sequential(
                    nn.Conv2d(in_channels, layer_size, kernel_size=kernel, stride=stride, padding=padding, bias=False),
                    nn.BatchNorm2d(layer_size),
                    nn.LeakyReLU()
                )
            )
            in_channels = layer_size
        self.fc_encoder = nn.Linear(cnn_layer_sizes[-1]*4, self.latent_dim)
        self.encoder = nn.Sequential(*modules)
        
        # Build the decoder
        self.fc_decoder = nn.Linear(self.latent_dim, cnn_layer_sizes[-1]*4)
        modules = []
        in_channels = cnn_layer_sizes[-1]
        for layer_size in reversed(cnn_layer_sizes):
            modules.append(
                nn.Sequential(
                    nn.ConvTranspose2d(in_channels, layer_size, kernel_size=kernel, stride=stride, padding=padding, bias=False),
                    nn.BatchNorm2d(layer_size),
                    nn.LeakyReLU()
                )
            )
        ...
        modules.append(
            nn.Sequential(
                nn.ConvTranspose2d(cnn_layer_sizes[0], cnn_layer_sizes[0], kernel_size=4,
                                   stride=1, padding=0),
                nn.LeakyReLU(),
                nn.ConvTranspose2d(cnn_layer_sizes[0], cnn_layer_sizes[0], kernel_size=3,
                                   stride=1, padding=0, bias=False),
                nn.BatchNorm2d(cnn_layer_sizes[0]),
                nn.LeakyReLU(),
                nn.Conv2d(cnn_layer_sizes[0], self.hparam['image_shape'][0], kernel_size=3, stride=1, padding=1),
                nn.Sigmoid())
            )
    ...
    def x_to_z(self, x, d=None):
        """Maps an image to z. 
        x.shape must be (batch_size, 1, self.hparam['image_shape'][1], self.hparam['image_shape'][2])"""
        encoding = self.encoder(x)
        encoding = torch.flatten(encoding, start_dim=1)
        return self.fc_encoder(encoding)

    def z_to_x(self, z, d=None):
        """Maps a latent vector to an image, where z ~ N(0, 1). 
        z.shape must be (batch_size, latent_dim)"""
        x = self.fc_decoder(z)
        x = x.view(-1, self.cnn_layer_sizes[-1], 2, 2)
        x = self.decoder(x)
        return x

    # def reparameterize(self, mu, log_var):
    #     """Samples a latent vector z from the distribution N(mu, var)"""
    #     std = torch.exp(0.5 * log_var)
    #     eps = torch.randn(std.size(), generator=self.generator, device=self.device)
    #     return mu + eps * std
...
class SimpleG(GDeep):
    ...
    def _build_models(self, hparam):
        n_final_decoder_channels = 64
        self.decoder_input_channels = n_final_decoder_channels
        self.encoder = nn.Sequential(
            nn.Conv2d(hparam['image_shape'][0], 64, kernel_size=4, stride=1, padding='same'),
            nn.LeakyReLU(),
            nn.Dropout2d(),
        ...
        self.fc_encoder = nn.Linear(n_final_decoder_channels*hparam['image_shape'][1]*hparam['image_shape'][2], self.latent_dim)

        self.fc_decoder = nn.Linear(self.latent_dim, n_final_decoder_channels*hparam['image_shape'][1]*hparam['image_shape'][2])
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(n_final_decoder_channels, 64, kernel_size=4, stride=1, padding=2),
            nn.LeakyReLU(),
            nn.Dropout2d(),
            nn.ConvTranspose2d(64, 64, kernel_size=4, stride=1, padding=2),
            nn.LeakyReLU(),
            nn.Dropout2d(),
            nn.ConvTranspose2d(64, hparam['image_shape'][0], kernel_size=3, stride=1, padding=0),
            nn.Sigmoid()
            )
    ...
    def z_to_x(self, z, d=None):
        ...
        z.shape must be (batch_size, latent_dim)"""
        x = self.fc_decoder(z)
        x = x.view(-1, self.decoder_input_channels, self.hparam['image_shape'][1], self.hparam['image_shape'][2])
        x = self.decoder(x)
        return x
...
class GMLP(nn.Module):
    def __init__(self, hparam):
        ...
        self.device = hparam['device']
        self.generator = torch.Generator(device=self.device).manual_seed(hparam['seed'])
        z_dim = hparam['latent_dim']

        x_dim = hparam['image_shape'][0] * hparam['image_shape'][1] * hparam['image_shape'][2]
        self.x_dim = x_dim
        h_dim1, h_dim2 = 512, 256
    ...
    def x_to_z(self, x, d=None):
        if len(x.shape) == 4:
            # x is an image, so flatten to a batch of vectors
    ...
    def z_to_x(self, z, d=None):
        h = F.relu(self.fc4(z))
        h = F.relu(self.fc5(h))
        return torch.sigmoid(self.fc6(h)).view(-1, *self.hparam['image_shape'])


class F_VAE_mlp(nn.Module):

    def __init__(self) -> None:
        super().__init__()
        raise NotImplementedError("F_mlp is not implemented yet")
...
class F_VAE_auto_spa_can(nn.Module):
    def __init__(self, hparam):
        ...
        super().__init__()
        self.device = hparam['device']
        self.n_domains = hparam['n_domains']
        self.latent_dim = hparam['latent_dim']
        self.generator = torch.Generator(device=self.device).manual_seed(hparam['seed'])
        # setting up projection from dim to S, L, and bias
        self._setup_models(hparam)

    def _setup_models(self, hparam):
        ...
        assert self.latent_dim > 0 and hparam['k_spa'] > 0, f'Must have N>=0 and k>=0, got N={self.latent_dim} and k={hparam["k_spa"]}'
        assert self.latent_dim > hparam['k_spa'], f'Must have N > k, got N={self.latent_dim} and k={hparam["k_spa"]}'
        ...
        self.L_nonshared_idxs = torch.tensor(
            sum([[(i,j) for j in range(i)] for i in range(self.latent_dim-hparam['k_spa'], self.latent_dim)], [])
        )
        # getting the indices of the diagonal matrix starting at row N-k
        self.S_nonshared_idxs = torch.tensor([[i, i] for i in range(self.latent_dim-hparam['k_spa'], self.latent_dim)])
        self.S_shared_idxs = torch.tensor([[i, i] for i in range(self.latent_dim-hparam['k_spa'])])
        ...
        self.d_to_nonshared_bias_dict = nn.ModuleDict({  # not shared across domains
            model_type: nn.Embedding(self.n_domains, hparam['k_spa']) 
                for model_type in ['mu', 'sigma', 'decode']
        })
        self.d_to_shared_bias_dict = nn.ParameterDict({  # shared across domains
            model_type: torch.nn.parameter.Parameter(torch.randn(self.latent_dim-hparam['k_spa']))
                for model_type in ['mu', 'sigma', 'decode']
        })
        return None
...
class F_VAE_auto_soft_can(F_VAE_auto_spa_can):
...
class F_VAE_auto_full(F_VAE_auto_spa_can):
    ...
    def _setup_models(self, hparam):
        ...
        self.d_to_domain_specific_bias_lookup = nn.Embedding(self.n_domains, hparam['latent_dim'])
        ...
        # setting up bias terms (these have to be in two separate dicts since one is a parameter and one is a module)
        self.d_to_nonshared_bias_dict = nn.ModuleDict({  # not shared across domains
            model_type: nn.Embedding(self.n_domains, hparam['latent_dim']) 
                for model_type in ['mu', 'sigma', 'decode']
        })
